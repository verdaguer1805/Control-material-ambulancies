...
test('TSNU has one automatic checklist and no consumption',()=>{const r=assignDemo({...legacy,service:'TSNU'});assert.equal(r.checklist,'TSNU');assert.equal(r.canConsume,false);assert.throws(()=>assignDemo({...legacy,service:'TSNU',checklist:'Polivalente'}));});
