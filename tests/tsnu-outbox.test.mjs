...
test('operations map to separate production RPCs',()=>{assert.equal(rpcForTsnuOperation({type:'withdrawal',operationId:'o',shiftId:'s',materials:{A:1}})[0],'append_tsnu_withdrawal');const manual=rpcForTsnuOperation({type:'finish',shiftId:'s',at:'2026-09-24T10:00:00Z'}),automatic=rpcForTsnuOperation({type:'finish',shiftId:'s',at:'2026-09-25T09:00:00Z',closeSource:'automatic'});assert.equal(manual[0],'finish_tsnu_shift_v2');assert.equal(manual[1].p_close_source,'manual');assert.equal(automatic[1].p_close_source,'automatic');assert.throws(()=>rpcForTsnuOperation({type:'bad'}));});
